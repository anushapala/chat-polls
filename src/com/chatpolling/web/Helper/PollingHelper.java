...
public class PollingHelper {
	...
	public static HashMap<String, Object> createPollHelper(HashMap<String, Object> pollDetailsMap) {
		...
		try {
			...
			HashMap<String, Object> pollQuestionDetails = (HashMap<String, Object>) pollDetailsMap
					.get("pollQuestionDetails");
			ArrayList<HashMap<String, Object>> pollOptionsDetails = (ArrayList<HashMap<String, Object>>) pollDetailsMap
					.get("pollOptionsDetails");
			...
			if (pollQuestionDetails != null && pollOptionsDetails != null && pollOptionsDetails.size() > 0) {
				...
				if (CommonUtil.isEmptyString(streamID) || CommonUtil.isEmptyString(createdBy)
						|| CommonUtil.isEmptyString(pollQuestion)) {
			...
			if (lstPollJDO != null && lstPollJDO.size() > 0) {
				for (PollJDO singlePollJDO : lstPollJDO) {
					...
					if (!CommonUtil.isEmptyString(singlePollJDO.getPollID())) {
						pollOptionsList = (ArrayList<PollItemJDO>) PollItemDAO
								.fetchPollOptionsForThisPollID(singlePollJDO.getPollID());
					} else {
